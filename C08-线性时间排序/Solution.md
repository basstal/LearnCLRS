...
e. 将已$k$排序好的数组看作$k$个有序数列，则问题转化为在$\Omicron(n\lg k)$时间内对$k$个有序数列进行归并。对这些序数列按首元素大小建最小堆，则该最小堆中总共有$k$个节点。每次从最小堆中取出堆顶元素值，并将该堆顶元素值赋值为对应数列的下一个元素。重建最小堆时间为$\Omicron(\lg k)$，重建最小堆的次数与元素总数量一致为$n$，因此通过该方法可以在$\Omicron(n\lg k)$时间内对一个长度为$n$的$k$排序数组进行全排序。
