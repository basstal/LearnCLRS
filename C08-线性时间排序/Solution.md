...
b. 因为在决策树中，每个非叶结点都代表着一次比较，因此LT和RT要么是叶结点，要么就是一颗有左子树和右子树的根结点。此处有三种情况：情况1，LT和RT均为叶结点，结论自然成立；情况2，LT和RT中有一个为叶结点，对于另一边的子树(RT)而言，根结点的所有叶结点的深度和 D(T) - 子树所有叶结点的深度和 D(RT) = 1 + 子树叶结点个数 = 根结点的叶结点个数 (k)，且有D(LT) = 0。结论成立。情况3，LT和RT均为子树，D(LT)+D(RT)其叶结点深度的和在其根结点的基础上总共减少了（根结点的）叶结点个数，结论成立。
...
![ThinkP8-3.png](Resources/ThinkP8_3.png)

A：

a. 先根据每个数的位长度，对数组进行桶排序，将相同位长度的数放在同一个桶中；然后对每个桶进行基数排序；最后按照已排序桶的顺序将结果串起来即可。

b. 根据每个字符串首字母的字典序，将字符串分别放在26个不同的桶中，并标记为1位；对已经放置在1位桶中的所有字符串，查看除标记位数以外的字符串首字母，如果有首字母则将其放置在26个不同的子桶中，并标记为2位，如果没有首字母则仍放置在原来的桶中。这样递归直到不产生子桶为止。从标记为1位的26个不同的桶中收集排序结果，先收集完每个桶的所有子桶后再收集下一个桶，即可得到排序结果。
...
![ThinkP8_4.png](Resources/ThinkP8_4.png)

A：

a. 对于$n$个红色水壶中的一个任意的水壶，与每一个蓝色水壶花一个单位时间比较，总共需要$n$个单位时间能匹配到蓝色水壶。剩余$n-1$个红色水壶重复执行此操作。因此整个匹配过程完成需要时间为$(2+n)(n-1)/2=\Theta(n^2)$

b. 实质上该问题可以看作对于由n个大小不同的数组成的数列的比较排序问题，每次都将其中一个未排序的数与其他未排序数进行比较，以确定这个数在数列中的排序位置。由比较排序问题的下界可知，解决该问题算法比较次数下界为$\Omega(n\lg n)$。
...
![ThinkP8-5.png](Resources/ThinkP8_5.png)
...
c. 由包含$n$个元素的数组A为$k$排序的性质：

$\frac{\sum_{j = i}^{i + k - 1}{A[j]}}{k} \leq \frac{\sum_{j=i+1}^{i+k}{A[j]}}{k}$

$A[i] + A[i + 1] + A[i + 2] + ... + A[i+k-1] \leq A[i+1]+A[i+2]+...+A[i+k]$

$A[i] \leq A[i+k]$，对所有的$i=1, 2, ..., n-k$成立。该公式反推也成立，因此得证。

d. 将待排序的数组看作$k$个子数组，其中相同子数组内每个元素的下标mod$k$后值相同，每个子数组包含$n/k$个元素，对子数组进行时间复杂度在$\Omicron(n/k\lg (n/k))$的比较排序，因此总的时间复杂度为$\Omicron(n\lg (n/k))$。对子数组的比较排序完成后，再按照每个子数组最小元素的大小顺序排序子数组，得到$k$排序的结果数组。

e. 将已$k$排序好的数组看作$k$个有序数列，则问题转化为在$\Omicron(n\lg k)$时间内对$k$个有序数列进行归并。对这些序数列按首元素大小建最小堆，则该最小堆中总共有$k$个结点。每次从最小堆中取出堆顶元素值，并将该堆顶元素值赋值为对应数列的下一个元素。重建最小堆时间为$\Omicron(\lg k)$，重建最小堆的次数与元素总数量一致为$n$，因此通过该方法可以在$\Omicron(n\lg k)$时间内对一个长度为$n$的$k$排序数组进行全排序。
...
![ThinkP8-6.png](Resources/ThinkP8_6.png)
...
![ThinkP8-7.png](Resources/ThinkP8_7.png)
