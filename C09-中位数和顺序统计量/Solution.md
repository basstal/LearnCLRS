...
MEDIAN(X, Y, n):
    if n == 1:
        ...
        let X1 be sub array of X from 1 to n/2
        let Y1 be sub array of Y from n/2 + 1 to n
    else
        let X1 be sub array of X from n/2 + 1 to n
        let Y1 be sub array of Y from 1 to n/2
    return MEDIAN(X1, Y1, n/2)
```
...
A：如果只有一口油井，则管道与油井y坐标相同为最短；两口油井时，则管道在两口油井的y坐标之间都是最短距离；三口油井时，则管道在其中一口油井的y坐标为最短，这口油井的y值夹在其他两口油井的y值区间内；四口油井时，情况与两口油井相同，只需要排除y更大或更小的那两口油井即可；依次类推我们可以知道只需要取所有油井y值的中位数即可得到主管道的最优位置。由SELECT算法可知，一组数值的中位数可以在线性时间内确定，那么该最优位置也可以在线性时间内确定。
...
![ThinkP9_1.png](./Resources/ThinkP9_1.png)

A：

a. 归并排序，算法运行时间为$\Omicron(n\lg n + i)$。

b. 构造最大堆，并且$i$次重新构造该最大堆，算法运行时间为$\Omicron(n\lg n + i\lg n)$。

c. SELECT算法找到第$i$大的元素，调用一次PARTITION划分，对划分后前$i$大的数进行随机快速排序，该算法运行时间为$\Omicron(2n + i\lg i)$。
...
![ThinkP9_2.png](./Resources/ThinkP9_2.png)

A：

a. 对于$x_1,x_2,...,x_n$的中位数，存在$(n-1)/2$个小于该元素的数，和$(n-1)/2$个大于该元素的数，且每个数的权重均为$1/n$，则所有小于中位数的元素权重和$(n-1)/2n\lt 1/2$，所有大于中位数的元素权重和$(n-1)/2n\leq 1/2$，因此该中位数就是带权中位数。

b. 对权重进行时间复杂度为$\Omicron(n\lg n)$的排序，并从小到大将权重累加直到总值$>0.5$，则最后一个累加值为$n$个元素的带权中位数。
...
![ThinkP9_3.png](./Resources/ThinkP9_3.png)
...
![ThinkP9_4.png](./Resources/ThinkP9_4.png)
