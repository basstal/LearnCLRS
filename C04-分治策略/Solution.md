...
A：递归式$T(n)=T(n-1)+1$对于第$i$层的子问题规模为1，每层节点均为1，深度为$n$，叶节点总数为1。因此有
...
A：递归式$T(n)=4T(\lfloor{n/2}\rfloor)+cn$对于第$i$层子问题规模为$n/2^i$，每层节点递增4，深度为$\lg n$，叶节点总数为$n^2$。因此有
...
A：递归式$T(n)=T(n-a)+T(a)+cn$对于第$i$层子问题规模为$cn-ica$，每层节点均为1，深度为$n/a$，叶节点总数为1。因此有
